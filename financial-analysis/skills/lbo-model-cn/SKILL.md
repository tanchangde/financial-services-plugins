---
name: lbo-model-cn
description: 此技能应用于完成私募股权交易的Excel LBO（杠杆收购）模型模板、交易材料或投资委员会演示。该技能填写公式、验证计算，并确保适应任何模板结构的专业格式标准。触发词："complete LBO model"（完成 LBO 模型）、"fill LBO template"（填写 LBO 模板）、"LBO analysis"（LBO 分析）、"private equity returns"（私募股权回报）或 "buyout model"（收购模型）。
---
...
### 平衡部分
* 当两部分必须相等时（例如来源 = 用途），通常其中一项作为"plug"（平衡项/轧差项）
* 识别哪一项是plug并计算为差额
...
| 循环引用错误 | 模型无法计算 | 对利息类计算使用期初余额，消除循环引用 |
