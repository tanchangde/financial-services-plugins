...
**如何引用假设 - 创建合并列：**
1. 情况选择器单元格（例如B6）包含1=熊市、2=基准或3=牛市
...
**每个合并列单元格都包含INDEX公式**，根据情况选择器从适当的情景块拉取。这使预测公式保持干净和可审计。
...
**然后创建一个合并列**（通常在右侧的下一列），使用INDEX公式根据情况选择器从选定的情景块拉取。这是您的预测公式引用的合并列。
